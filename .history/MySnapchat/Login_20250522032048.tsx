...
import { AuthContext } from './Auth'; // ou '../Token' si tu l’as renommé
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.put(
        ...
        {
          headers: {
            'x-api-key': 'TA_CLE_API_ICI', // remplace ici
          },
        }
      );
      ...
      if (token) {
        login(token); // met à jour le contexte, donc redirige vers Home
      } else {
